...
type TypeMeta = { label: string; icon: keyof typeof Ionicons.glyphMap; color: string; bg: string };

// "appointment_confirmed" acompanha a marca da clínica; os demais tipos são
// cores semânticas fixas (lembrete, cancelamento, remarcação, mensagem, aviso).
const buildTypeMeta = (colors: ThemeColors): Record<string, TypeMeta> => ({
...
const getNotificationMeta = (type: string, colors: ThemeColors) => {
  const meta = buildTypeMeta(colors);
